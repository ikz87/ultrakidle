...
async function removeReaction(channelId: string, messageId: string, emoji: string) {
  const encoded = encodeURIComponent(emoji);
  await discordFetch(
    `${DISCORD_API}/channels/${channelId}/messages/${messageId}/reactions/${encoded}/@me`,
...
async function approveSubmission(
  ...
  const msgRes = await discordFetch(
    `${DISCORD_API}/channels/${sub.channel_id}/messages/${sub.message_id}`,
    { headers: discordHeaders }
  );
  ...
  if (!imgRes.ok) return { action: "skipped", reason: "download_failed" };
  ...
  try {
    ...
    console.error(`[resolve] R2 Upload error #${sub.id}:`, err);
  ...
  await supabase
    ...
    .update({
      ...
      image_url: `${GALLERY_DOMAIN}/${storagePath}`,
  ...
  await removeReaction(sub.channel_id, sub.message_id, "👀");
  await addReaction(sub.channel_id, sub.message_id, "✅");
  await sendMessage(sub.channel_id, "✅ **Submission approved!** Added to gallery.");
  await closeThread(sub.channel_id);
...
serve(async (req) => {
  ...
  if (!submissions?.length) {
    return Response.json({ results: [], approved: 0, rejected: 0, expired: 0, skipped: 0 });
  }
  ...
  if (!levels?.length) return Response.json({ error: "No levels found" }, { status: 500 });

  const levelMap = new Map(levels.map((l) => [l.level_number, l.id]));
  const levelNumMap = new Map(levels.map((l) => [l.id, l.level_number]));
  const now = Date.now();

  const results: any[] = [];
  let [approved, rejected, expired, skipped] = [0, 0, 0, 0];
  ...
  for (const sub of submissions) {
    try {
      ...
      if (force_approve) {
        const result = await approveSubmission(supabase, sub, levelNumMap);
      ...
      const threadRes = await discordFetch(`${DISCORD_API}/channels/${sub.channel_id}`, {
        headers: discordHeaders,
      });
      if (!threadRes.ok) {
        results.push({ id: sub.id, action: "skipped", reason: "thread_fetch_failed" });
        skipped++;
        continue;
      }
      ...
      if (!levelId) {
        ...
        await sendMessage(
          sub.channel_id,
          `❌ **Submission rejected** — Title invalid level: "${currentTitle}"`
        );
        await closeThread(sub.channel_id);
        results.push({ id: sub.id, action: "rejected", reason: "invalid_title_edit" });
        rejected++;
        continue;
      }

      if (levelId !== sub.level_id) {
        await supabase.from("image_submissions").update({ level_id: levelId }).eq("id", sub.id);
      ...
      if (down >= REACTION_THRESHOLD && down >= up) {
        ...
        await closeThread(sub.channel_id);
        results.push({ id: sub.id, action: "rejected", reason: "vote" });
        rejected++;
        continue;
      }
      ...
      if (now - createdAt > STALE_MS) {
        await supabase
          ...
          .eq("id", sub.id);
        await removeReaction(sub.channel_id, sub.message_id, "👀");
        await sendMessage(sub.channel_id, "⏰ **Submission expired**.");
        await closeThread(sub.channel_id);
      ...
      results.push({ id: sub.id, action: "skipped", reason: `${up}👍 ${down}👎` });
      skipped++;
    } catch (e) {
      results.push({ id: sub.id, action: "error", reason: String(e) });
      rejected++;
    }
  }

  return Response.json({ results, approved, rejected, expired, skipped });
});
